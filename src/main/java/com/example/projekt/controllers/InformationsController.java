...
public class InformationsController {
    ...
    @GetMapping("/")
    public String getInformationsGet(@AuthenticationPrincipal UserDetails loggedUser,
        ...
        if (categoryName != null && !categoryName.isEmpty()) {
            informations = informationService.getInformationRepository()
                    .findAllByCategory_NameAndLoginAndShared(categoryName, Sort.by(direction, sortField),loggedUser.getUsername(),false);
        } else {
            if (sortField.equals("categoryOccurrences")) {
                informations = informationService.getAllInformationsSortedByCategoryOccurrences(Sort.by(direction, sortField),loggedUser.getUsername(),false);
            } else {
                informations = informationService.getInformationRepository()
                        .findAllByLoginAndShared(loggedUser.getUsername(), Sort.by(direction, sortField),false);
            }
        }
    ...
    @PostMapping("/")
    public String getInformationsPost(@AuthenticationPrincipal UserDetails loggedUser,
            @Valid @ModelAttribute Information information, BindingResult bindingResult, Model model) {
    ...
    @GetMapping("/category")
    public String getInformationsFromCategory(@AuthenticationPrincipal UserDetails loggedUser,@RequestParam("categoryName") String category, Model model){
        Category categoryObj = categoryService.getCategoryRepository().findByName(category);
        if (categoryObj != null) {
            model.addAttribute("informations", informationService.getInformationRepository().findAllByCategory_NameAndLoginAndShared(categoryObj.getName(), Sort.by(Sort.Direction.ASC, "creationTime"),loggedUser.getUsername(),false));
        } else {
            model.addAttribute("informations", informationService.getInformationRepository().findAll());
        }
        return "informations";
    ...
    @GetMapping("/shared")
    public String getInformationsSharedGet(@AuthenticationPrincipal UserDetails loggedUser,
                                     @RequestParam(required = false) String categoryName,
                                     @RequestParam(required = false) String sortBy,
                                     @RequestParam(required = false) String sortDirection,
                                     @RequestParam(required = false) String filterByStartDate,
        ...
        if (resetFilters) {
            ...
            removeCookie(response, "filterByStartDate");
            removeCookie(response, "filterByEndDate");
            return "redirect:/informations/shared";
        }
        ...
        if (categoryName != null && !categoryName.isEmpty()) {
            informations = informationService.getInformationRepository()
                    .findAllByCategory_NameAndLoginAndShared(categoryName, Sort.by(direction, sortField),loggedUser.getUsername(),true);
        } else {
            if (sortField.equals("categoryOccurrences")) {
                informations = informationService.getAllInformationsSortedByCategoryOccurrences(Sort.by(direction, sortField),loggedUser.getUsername(),true);
            } else {
                informations = informationService.getInformationRepository()
                        .findAllByLoginAndShared(loggedUser.getUsername(), Sort.by(direction, sortField),true);
            }
        }
        ...
        model.addAttribute("informations", informations);
        model.addAttribute("categories", categoryService.getCategoryRepository().findAllByLogin(loggedUser.getUsername()));
        return "shared-informations";
    }

    @PostMapping("/shared")
    public String getInformationsSharedPost(@AuthenticationPrincipal UserDetails loggedUser,
                                      @Valid @ModelAttribute Information information, BindingResult bindingResult, Model model) {
        if (bindingResult.hasErrors()) {
            model.addAttribute("errors", bindingResult.getAllErrors());
            model.addAttribute("information", information);
